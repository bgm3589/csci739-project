...
def select_uv(i, j, n, depth, color, device='cuda:0'):
    ...
    """
    i = i.reshape(-1).to(device)
    j = j.reshape(-1).to(device)
...
def select_uv_semantic_segment(i, j, n, depth, color, yolo_model, device='cuda:0'):
    """
    Select n uv from dense uv.
    ...
    results = yolo_model.predict(color_np, verbose=False)
    sel = results[0].boxes.cls == 56.


    mask = np.full(results[0].orig_shape, False)
    if results[0].masks != None:
        for msk in results[0].masks.data[sel]:
            msk = msk.cpu().numpy()
            msk = cv2.resize(msk, (color.shape[1], color.shape[0]))
    ...
    if cntr % 100 == 0:
        ...
        imgs.append((color_np, mask))
        axarr[0].imshow(color_np)
        axarr[1].imshow(mask)
        f.savefig("show.png")
    if cntr % 1000 == 0:
        f, axarr = plt.subplots(2,1) 
        im1 = axarr[0].imshow(imgs[0][0], animated=True)
        im2 = axarr[1].imshow(imgs[0][1], animated = True)
        im = [im1,im2]

        def update(i):
            im[0].set_array(imgs[i][0])
            im[1].set_array(imgs[i][1])
            return im
        animation_fig = animation.FuncAnimation(f, update, frames=len(imgs), interval=200, blit=True,repeat_delay=10,)
        animation_fig.save("show_ani.gif")

    i = i[mask != True]
    j = j[mask != True]
    color = color[mask != True]
    depth = depth[mask != True]
    # print(i.shape, j.shape, color.shape, depth.shape)
...
def get_sample_uv(H0, H1, W0, W1, n, depth, color, device='cuda:0', yolo_model = None):
    """
    ...
    i, j = torch.meshgrid(torch.linspace(
        W0, W1-1, W1-W0), torch.linspace(H0, H1-1, H1-H0))
    i = i.t()  # transpose
    j = j.t()
    if yolo_model == None:
        i, j, depth, color = select_uv(i, j, n, depth, color, device=device)
    else:
        i, j, depth, color = select_uv_semantic_segment(i, j, n, depth, color, yolo_model, device=device)

    return i, j, depth, color


def get_samples(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2w, depth, color, device, yolo_model=None):
    """
    ...
    i, j, sample_depth, sample_color = get_sample_uv(
        H0, H1, W0, W1, n, depth, color, device=device, yolo_model=yolo_model)
...
def get_tensor_from_camera(RT, Tquad=False):
    ...
    # quad = quaternion.as_float_array(quad)
    # print(quad)
    if Tquad:
